document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBox.addEventListener('input', function() {
        ...
        const filteredProducts = Object.values(product).flatMap(category =>
            category.filter(item => item.title.toLowerCase().includes(searchTerm))
        );
        ...
        products.forEach(item => {
            ...
            <div class='box'>
                ...
                <div class='bottom'>
                    <p>${title}</p>
                    <h2>$ ${price}</h2>
                    <button class="add-to-cart-btn" data-index="${id}" data-type="${getTypeById(id)}">Add to cart</button>
                </div>
            </div>`; 
        });
        //Attach event listeners for add to cart buttons
        attachAddToCartListeners(root); // Pass root element to attachAddToCartListeners
    }

    // Function to determine the type by ID
    function getTypeById(id) {
        for (const key in product) {
            if (product[key].some(item => item.id === id)) {
                return key;
            }
        }
        return null;
    ...
    function attachAddToCartListeners(root) {
        root.querySelectorAll('.add-to-cart-btn').forEach(button => {
            button.addEventListener('click', function() {
                const index = parseInt(this.getAttribute('data-index'));
                const type = this.getAttribute('data-type');
                console.log('Adding to cart:', index, type);
    ...
    function addtocart(index, type) {
        const selectedItem = product[type].find(item => item.id === index);
        const isItemInCart = cart.some(item => item.type === type && item.id === index);

        if (!isItemInCart) {
            cart.push({ ...selectedItem, type });
            displaycart();
        } else {
    ...
    renderProducts(Object.values(product).flat());
    updateCartCount();
    displaycart();
});
